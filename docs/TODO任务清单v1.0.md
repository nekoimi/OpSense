...
- [ ] `Must` 使用 `ssh2` 实现 SSH 连接。
- [ ] `Must` 支持 SSH Agent。
- [ ] `Must` 支持私钥文件。
- [ ] `Should` 支持交互式密码输入，但不得记录密码。
- [ ] `Must` 支持连接超时和 keepalive。
- [ ] `Must` 默认启用主机指纹校验。
- [ ] `Must` 对首次连接和指纹变化给出明确处理方式。

### M2-02 统一命令执行器

- [ ] `Must` 定义 `CommandSpec`。
- [ ] `Must` 定义命令 ID、固定模板、允许参数、超时和最大输出大小。
- [ ] `Must` 返回 stdout、stderr、退出码、执行时间和截断状态。
- [ ] `Must` 支持命令级超时和取消。
- [ ] `Must` 禁止将任意用户输入直接拼接为 shell 命令。
- [ ] `Must` 记录命令审计信息，但不记录敏感参数。

### M2-03 只读命令允许列表

- [ ] `Must` 建立系统、存储、网络、服务和目录扫描命令清单。
- [ ] `Must` 为每条命令标记支持的发行版和依赖命令。
- [ ] `Must` 为每条命令标记是否需要 `sudo`。
- [ ] `Must` 禁止启停服务、修改文件、安装软件、修改用户和磁盘操作。
- [ ] `Must` 增加单元测试，证明外部输入不能突破命令模板。

### M2-04 权限探测

- [ ] `Must` 获取当前用户、UID 和用户组。
- [ ] `Must` 探测 `sudo -n` 是否可用。
- [ ] `Must` 输出 `unprivileged`、`partial_privileged` 或 `privileged`。
- [ ] `Must` 权限不足时继续扫描其他项目。
- [ ] `Must` 在最终报告中展示权限缺口。
