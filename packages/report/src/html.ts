  ...
  const rows = model.services
    .flatMap((service, index) =>
      service.reportPlacement !== placement
        ? []
        : [
            `<tr><td><a href="#service-${index}">${escapeHtml(service.displayName ?? service.name)}</a></td><td>${badge(service.status)}</td><td>${escapeHtml(service.deploymentType)}</td><td>${escapeHtml(roleLabel(service.role))}</td><td>${escapeHtml(displayList(service.ports))}</td></tr>`,
          ],
    )
    .join('');
  return `<h3>${escapeHtml(title)}</h3>${table(['服务', '状态', '部署方式', '角色', '监听端口'], rows)}`;
}
  ...
  return `<details id="service-${index}" class="service" ${service.status === 'failed' ? 'open' : ''}>
    ...
    <div class="service-body">
      ...
      <div class="service-facts">
        ...
        ${factGroup('网络', [
          ['监听端口', displayList(service.ports)],
...
function factGroup(title: string, rows: ReadonlyArray<readonly [string, string]>): string {
  return `<article><h4>${escapeHtml(title)}</h4><dl>${rows
    .map(
      ([label, value]) =>
        `<div><dt>${escapeHtml(label)}</dt><dd>${shouldUseCode(label) ? `<code>${escapeHtml(value)}</code>` : escapeHtml(value)}</dd></div>`,
    )
    .join('')}</dl></article>`;
}
